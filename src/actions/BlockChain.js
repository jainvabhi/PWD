...
export const getPatient = (patientName) => dispatch => {
  const web3 = new Web3(new Web3.providers.HttpProvider('http://demo1s62x.southcentralus.cloudapp.azure.com:8545'));

  const account = web3.eth.getAccounts((err, accs) => accs[0]);
  web3.eth.defaultAccount = account;

  const source = 'pragma solidity ^0.4.6;contract PatientSafe{struct PatientDetails{string patientName;uint patientId;uint previousPatientId;uint Timestamp;string bloodGroup;}mapping(uint => PatientDetails) Trail;uint8 TrailCount=0;function AddNewPatient(uint patientId, string patientName, string bloodGroup){PatientDetails memory newPatient;newPatient.patientName = patientName;newPatient.patientId = patientId;newPatient.bloodGroup= bloodGroup;newPatient.Timestamp = now;if(TrailCount!=0){newPatient.previousPatientId = Trail[TrailCount].patientId;}Trail[TrailCount] = newPatient;TrailCount++;}function GetTrailCount() returns(uint8){return TrailCount;}function GetPatient(uint8 TrailNo) returns(string,uint,uint,uint,string){ return (Trail[TrailNo].patientName, Trail[TrailNo].patientId,Trail[TrailNo].previousPatientId,Trail[TrailNo].Timestamp ,Trail[TrailNo].bloodGroup); }}';
  ...
  web3.eth.compile.solidity(source, (error, safeCompiled) => {
    ...
    deployedSafe.GetTrailCount.call((error, trailCount) => deployedSafe.GetPatient.call(trailCount - 1, (err, returnValues) => {
      const data = {
        'name': CryptoJS.AES.decrypt(returnValues[0], '123').toString(CryptoJS.enc.Utf8),
        'bloodGroup': CryptoJS.AES.decrypt(returnValues[4], '123').toString(CryptoJS.enc.Utf8),
      };
...
export const getPhiDetail = (patientName) => dispatch => {
  ...
  web3.eth.compile.solidity(source, (error, safeCompiled) => {
    ...
    deployedSafe.GetTrailCountPHI.call((error, trailCount) => deployedSafe.GetPatientPHI.call(trailCount - 1, (err, returnValues) => {
      const blood = CryptoJS.AES.decrypt(returnValues[2], '123').toString(CryptoJS.enc.Utf8);
      const biling = CryptoJS.AES.decrypt(returnValues[3], '123').toString(CryptoJS.enc.Utf8);
      const drug = CryptoJS.AES.decrypt(returnValues[4], '123').toString(CryptoJS.enc.Utf8);
      const hiv = CryptoJS.AES.decrypt(returnValues[5], '123').toString(CryptoJS.enc.Utf8);
      const genetic = CryptoJS.AES.decrypt(returnValues[6], '123').toString(CryptoJS.enc.Utf8);
