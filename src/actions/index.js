...
export const loginWithToken = () => (dispatch, getState) => {
  ...
  axios
    .post(
      'https://hackthonfunction.azurewebsites.net/api/PersonDetail?code=m8bz7hSfwXOyE3epgBnVEpDwFV5d8WwiQFoJ0Bhp14uRwKpEysbQWQ==',
      { personId: token },
    )
    .then(personResponse => {
      const data = {
...
export const uploadAzure = file => dispatch => {
  ...
  axios
    .post(
      '//hackthonapp.azurewebsites.net/api/Upload/user/PostUserImage',
    ...
    .then(response => {
      ...
      axios
        .post(
          'https://hackthonfunction.azurewebsites.net/api/HttpTriggerJS1?code=RfbEiaecqduava7lOVX0LNxPQrRVJjXIpoH8MB3J9iaI6wFm5IXQJg==',
          faceObj,
        )
        .then(faceResponse => {
          let identifyObj = {
          ...
          axios
            .post(
              'https://hackthonfunction.azurewebsites.net/api/Identify?code=0a1w8ALPxSFPERpnQv4gCpz44IYV2duGQEXilq4/hymRWRePGnRdKQ==',
              identifyObj,
            )
...
export const backToDashboard = () => dispatch => {
  dispatch({ type: types.SHOW_DASHBOARD });
};
